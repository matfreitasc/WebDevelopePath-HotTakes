...
export class SingleSauceComponent implements OnInit {
  ...
  onLike() {
    ...
    this.sauce$
      .pipe(
        take(1),
        switchMap((sauce: Sauce) =>
          this.sauces.likeSauce(sauce._id, !this.liked).pipe(
            tap((liked) => {
  ...
  onDislike() {
    ...
    this.sauce$
      .pipe(
        take(1),
        switchMap((sauce: Sauce) =>
          this.sauces.dislikeSauce(sauce._id, !this.disliked).pipe(
            tap((disliked) => {
              this.likePending = false;
            ...
            map((disliked) => ({
              ...sauce,
              likes: disliked ? sauce.dislikes + 1 : sauce.dislikes - 1,
            })),
            tap((sauce) => (this.sauce$ = of(sauce)))
          )
        )
